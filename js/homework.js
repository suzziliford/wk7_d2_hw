...
class User{
    ...
    changePassword(password, newPassword){
        if (this.password === password){
            console.log(`Your new password has been set to ${newPassword}`);
            this.password = newPassword;
        } else {
            console.log('Password entry incorrect. Your password has not been changed')
        }
    }
}

let reno = new User('Reno', 'fork33ps')
console.log(reno);

reno.changePassword('fork33ps','fork3ats');
...
let squareNegatives = [-5, -4, -3, -2, -1, 1, 2, 3, 4, 5] // [25, 16, 9, 4, 1]
let total = 0;
let negsSq = squareNegatives.filter(function(x){
    if (x < 0) {
        x = x*x;
        total += x;
        return total;
